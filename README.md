...
![UM-ProtoShare Workflow](images/workflow_01.png)
---
### 🧠 Core ideas
...
- **Prototype scale allocation (multi-scale prototypes)**
  ...
  - scale 3 (coarsest feature map) → 20% of K  
  - scale 2 (mid-level feature map) → 50% of K  
  - scale 1 (finest feature map) → 30% of K
  
  Changing `--num-prototypes` on the command line changes the total number of prototypes K, and the model will automatically recompute the per-scale counts using the 30/50/20 split.
...
```bibtex
@inproceedings{
golbaf2026umprotoshare,
title={{UM}{\textendash}ProtoShare: {UN}et{\textendash}Guided, Multi{\textendash}Scale Shared Prototypes for Interpretable Brain Tumour Classification Using Multi{\textendash}Sequence 3D {MRI}},
